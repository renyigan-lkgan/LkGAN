...
k, version, trial_number, seed_num = input("k, version, trial number, seed: ").split()
if int(version) == 1:
    alpha = 0.6
    beta = 0.4
elif int(version) == 2:
...
class GAN(object):
    def __init__(self, k, version, alpha, beta, gamma, trial_num):
        self.batch_size = 100
        self.n_classes = 10
        ...
        self._make_directory('data/v_' + str(self.version))
        self._make_directory('data/v_' + str(self.version) + '/k_' + str(self.k))
        self.save_path = 'data/v_' + str(self.version) + '/k_' + str(self.k) + '/trial' + str(self.trial_num)
    ...
    def gen_loss(self):
        return tf.math.reduce_mean(tf.math.pow(tf.math.abs(self.fake_output - self.gamma),
                                               self.k * tf.ones_like(self.fake_output)))
    ...
    def train_one_epoch(self, sess, init, epoch):
        ...
        try:
            ...
            pass
        current_fid = self.calculate_fid(sess)
    ...
    def save_generated_images(self, epoch, sess):
        ...
        np.save(self.save_path + '/predictions', self.predictions)

    def train(self, n_epochs):
        # If you want to save checkpoints, uncomment the code below
        #self._make_directory('checkpoints')
        #self._make_directory('checkpoints/v_' + str(self.version))
        #self.cpt_PATH = 'checkpoints/v_' + str(self.version) + '/k_' + str(self.k)
        #if self.trial_num == 1:
...
model = GAN(round(float(k), 1), int(version), int(alpha), int(beta), int(gamma), int(trial_number))
model.build()
